...
def add_employee():
    ...
    db.execute('''INSERT INTO employees
        (id,name,desig,dept,doj,salary,phone,email,address,city,pin,bank,bank_name,ifsc,acc_name,cl,sl,el,status)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
...
def update_employee(emp_id):
    ...
    db.execute('''UPDATE employees SET
        name=?,desig=?,dept=?,doj=?,salary=?,phone=?,email=?,
        address=?,city=?,pin=?,bank=?,bank_name=?,ifsc=?,acc_name=?,cl=?,sl=?,el=?
        WHERE id=?''',
...
def get_attendance(month_key):
    db = get_db()
    rows = db.execute(
        "SELECT emp_id, day, status FROM attendance WHERE month_key=?",
        (month_key,)).fetchall()
    ot_rows = db.execute(
        "SELECT emp_id, ot_days FROM ot_days WHERE month_key=?",
        (month_key,)).fetchall()
    db.close()
...
def calculate_payroll():
    ...
    for emp in emps:
        ...
        if not calc:
            continue
        existing = db.execute(
            "SELECT paid,paid_date,posted_to_ledger FROM payroll WHERE emp_id=? AND month_key=?",
        ...
        db.execute('''INSERT INTO payroll
            ...
            extra_ot=excluded.extra_ot, total_ot=excluded.total_ot,
...
def post_to_ledger():
    ...
    db = get_db()
    pr = db.execute("SELECT * FROM payroll WHERE emp_id=? AND month_key=?",
        (emp_id, month_key)).fetchone()
    if not pr:
        db.close()
    ...
    db.execute("UPDATE payroll SET posted_to_ledger=1 WHERE emp_id=? AND month_key=?",
        (emp_id, month_key))
    db.commit()
    db.close()
    return jsonify({'ok': True})
...
def mark_paid():
    ...
    db = get_db()
    pr = db.execute("SELECT net FROM payroll WHERE emp_id=? AND month_key=?",
        (emp_id, month_key)).fetchone()
...
def get_ledger(emp_id):
    db = get_db()
    rows = db.execute("SELECT * FROM ledger WHERE emp_id=? ORDER BY date,id",
        (emp_id,)).fetchall()
    db.close()
    return jsonify([dict(r) for r in rows])
